...
namespace ExF_MerchantName_Rename
{
    internal class Program
    {
        ...
        //インフォルダパスの宣言
        //public static string InFolderPath = "";
        ...
        static int Main(string[] args)
        {
            ...
            {
                ErrMsg = "対象のフォルダ内にファイルが1件も見つかりませんでした。";
                CreateLog(ErrMsg);
                MessageBox.Show(ErrMsg, "エラー",
                MessageBoxButtons.OK,
                MessageBoxIcon.Error,
            ...
            foreach (string file in outfiles)
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    if (splitline[4].Length == 0)
                    {
                        //新規の想定                       
                        //NULLなら書き換え不要。そのまま新しい行を作成する。実際のファイルへの書き込みはファイルを閉じてから行う。
                        AllLines.Add(line);
                    }
                    else if (splitline[4].Length == 20)
                    {
                        //端末追加・ブランド追加の想定。加盟店を書き換える必要あり
                        //加盟店コードを見てdic_Outに含まれていれば
                        if (dic_Out.Contains(splitline[4]))
                        {

                            if (splitline[15].ToString() == dic_Out[splitline[4]].ToString())
                            {
                                //加盟店名が一致しているため何もしない
                                AllLines.Add(line);
                            }
                            else
                            {
                                ...
                                ChangeFLG = "1";
                                CreateLog($"依頼書 {No} レコード目：加盟店コード({splitline[4]})「{splitline[15]}」→ DB：「{dic_Out[splitline[4]]}」");

                                //加盟店名が一致していないためDBの加盟店名に書き換える。設置店名も書き換える。加盟店名=設置店名の想定。
                                splitline[15] = dic_Out[splitline[4]].ToString();//加盟店名
                                splitline[17] = dic_Out[splitline[4]].ToString();//設置店名
